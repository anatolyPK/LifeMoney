...
from backend.src.base.base_model import (
    Share,
    Future,
    Currency,
    Etf,
...
from backend.src.modules.cryptos.schemas import TransactionAdd
...
from backend.src.modules.stocks.exceptions import MissingTransactionIdError
from backend.src.modules.stocks.utils import convert_tinkoff_money_in_currency, convert_str_to_datetime
...
class StockTransactionsRepository(
        ...
        for id_key, (schema, repository) in transaction_map.items():
            if getattr(transaction, id_key) > 0:
                transaction_instance = schema(
                    **transaction_data, asset_id=transaction_data.get(id_key)
                )
    ...
    async def get_user_transaction(
        self, order: str = "timestamp", limit: int = 100, offset: int = 0, **filters
...
class BaseTransactionRepository(
    SqlAlchemyRepository[ModelType, TransactionAdd, TransactionAdd], Generic[ModelType]
):
    async def get_transactions(
        self, order: str = "id", limit: int = 100, offset: int = 0, **filters
    ) -> list[ReadTransactionSchema]:
        results: list[ModelType] = await super().get_multi(
            order, limit, offset, **filters
        )
        return [
            ReadTransactionSchema.model_validate(transaction) for transaction in results
        ]
...
class BaseAssetRepository(
    ...
    async def create_multi(self, data: list[dict[str, Any]]):
        existing_figis = await self.check_existing_records(
            "figi", [data_dict["figi"] for data_dict in data]
        )
        orm_data = [
            self.create_asset(data_dict)
            for data_dict in data
            if data_dict["figi"] not in existing_figis
        ]
...
class BondRepository(BaseAssetRepository[Bond]):
    def create_asset(self, data_dict: dict[str, Any]) -> Bond:
        return Bond(
            ...
            exchange=data_dict["exchange"],
            nominal=convert_tinkoff_money_in_currency(data_dict["nominal"]),
            initial_nominal=convert_tinkoff_money_in_currency(
                data_dict["initialNominal"]
            ),
...
class CurrencyRepository(BaseAssetRepository[Currency]):
    def create_asset(self, data_dict: dict[str, Any]) -> Currency:
        return Currency(
            ...
            nominal=convert_tinkoff_money_in_currency(data_dict["nominal"]),
            country_of_risk=data_dict["countryOfRisk"],
            min_price_increment=convert_tinkoff_money_in_currency(
                data_dict["minPriceIncrement"]
            ),
        )
...
share_repository = ShareRepository(
    model=Share, db_session=db_helper.get_db_session_context, dto_schema=ShareSchema
)

bond_repository = BondRepository(
    model=Bond, db_session=db_helper.get_db_session_context, dto_schema=BondSchema
)

etf_repository = EtfRepository(
    model=Etf, db_session=db_helper.get_db_session_context, dto_schema=EtfSchema
)
...
future_repository = FutureRepository(
    model=Future, db_session=db_helper.get_db_session_context, dto_schema=FutureSchema
)
